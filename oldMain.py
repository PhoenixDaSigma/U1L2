...
def select(rats, chonkHallOfFame, smolBeans):
  rats2 = [[], []]
  for i in rats:
    for j in i:
      if j.canBreed:
  ...
  if mList[-1] < fList[-1]:
    smolBeans.append(mList[-1])
  else:
    smolBeans.append(fList[-1])
  ...
  return masterRace, chonkHallOfFame, smolBeans
...
def ldMain():
  ...
  while potentialMean < GOAL and generations < GENERATION_LIMIT:
    children = breed(rats)
    rats, chonkHallOfFame, smolBeans = select(children, chonkHallOfFame, smolBeans)
  ...
  for i in range(len(meanList)):
    meanList[i] = str(meanList[i])
  with open("meanFile.txt", "w") as openMean:
    openMean.write('\n'.join(meanList))
  
  for i in range(len(chonkHallOfFame)):
    chonkHallOfFame[i] = str(chonkHallOfFame[i])
  with open("bigFile.txt", "w") as openBig:
    openBig.write('\n'.join(chonkHallOfFame))
  
  for i in range(len(smolBeans)):
    smolBeans[i] = str(smolBeans[i])
  with open("smallFile.txt", "w") as openSmall:
    openSmall.write('\n'.join(smolBeans))
